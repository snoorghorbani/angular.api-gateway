...
angular
    ...
    .provider('apiGateway', function () {
        ...
        window.$$$api_debug_management = (function () {
            var cache = {
                actions_that_must_pause_before_send_request: []
            };
            var need_pause_before_send = function need_pause_before_send(actionInstance) {
                if (actionInstance.options.pause_before_send) return true;
                if (cache.actions_that_must_pause_before_send_request.indexOf(actionInstance.options.actionName) > -1) return true
            }
            var pause_before_send = function pause_before_send(actionName) {
                cache.actions_that_must_pause_before_send_request.push(actionName);
            }
            return {
                need_pause_before_send: need_pause_before_send,
        ...
        return {
            ...
            $get: ['$rootScope', '$state', '_', "$resource", "$q", function ($rootScope, $state, _, $resource, $q) {
                ...
                apiGateway.context = function (contextName) {
                                ...
                                Fn.$promise = {
                                    ...
                                    "catch": function (catchCallback) {
                                        ...
                                        if (itemIndex == -1) {
                                            ...
                                            _db[contextName][actionName]._config.catch[parseInt(itemIndex)] = catchCallback;
                                        }
                                        return this;
                                    }
                                }
                        ...
                        _db[contextName][methodName] = function (actionInstance/*args*/) {
                            ...
                            if (actionInstance.options.cache.expiredTime && loadFromLocalStorage && loadFromLocalStorage.isFresh) {
                                ...
                                if (_model[contextName][methodName]) {
                                    ...
                                    if ($$$api_debug_management.need_pause_before_send(actionInstance)) debugger;
                    ...
                    var add_api = function (contextName, actionName, methodType, route, routeExtending) {
                        this.actionName = actionName;
                        var actions = {}
                        actions[actionName] = { method: methodType, params: {} };
                        var api;
                        if (route && !routeExtending) {
                            api = $resource('/' + route, {}, actions);
                        } else if (route && routeExtending) {
                    ...
                    var route = function (route, extend) {
                        options.route = route;
                        options.routeExtending = extend || false;
                        return this;
                    };
                    ...
                    var done = function () {
                        ...
                        add_api(options.context, options.actionName, options.methodType, options.route, options.routeExtending);
